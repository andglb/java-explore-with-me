...
public class CommentServiceImpl implements CommentService {
    ...
    public static final String COMMENT_WAS_UPDATE = "Comment with ID = {} was update";
    ...
    public CommentDto updateCommentByUser(NewCommentDto newCommentDto, Long userId, Long commentId) {
        Comment oldComment = commentRepository.findById(commentId).orElseThrow(() -> new CommentNotExistException("Can't update comment, comment doesn't exist"));
        if (!userRepository.existsById(userId)) {
            throw new UserNotExistException("Can't update comment, if user doesn't exist");
        }
        if (!oldComment.getAuthor().getId().equals(userId)) {
            throw new CommentConflictException("Can't update comment, if his owner another user");
        }
        oldComment.setText(newCommentDto.getText());
        Comment savedComment = commentRepository.save(oldComment);
        log.debug(COMMENT_WAS_UPDATE, commentId);
    ...
    public CommentDto getCommentsByIdByUser(Long userId, Long commentId) {
        Comment comment = commentRepository.findById(commentId).orElseThrow(() -> new CommentNotExistException("Can't get comment"));
        if (!userRepository.existsById(userId)) {
    ...
    public void deleteCommentByUser(Long userId, Long commentId) {
        Comment comment = commentRepository.findById(commentId)
                .orElseThrow(() -> new CommentNotExistException("Can't delete comment, if his owner another user or user/comment doesn't exist"));
        if (!userRepository.existsById(userId)) {
    ...
    public CommentDto getCommentsByIdByAdmin(Long commentId) {
        Comment comment = commentRepository.findById(commentId)
                .orElseThrow(() -> new CommentNotExistException(String.format("Can't receive comment by id, the comment with id=%s doesn't exist", commentId)));
    ...
    public void deleteCommentByAdmin(Long commentId) {
        commentRepository.findById(commentId)
                .orElseThrow(() -> new CommentNotExistException(String.format("Can't delete comment, the comment with id=%s doesn't exist", commentId)));
